...
public class UserServiceImpl implements UserService {
    ...
    private static UserAccount toUserAccount(UserAccountEntity accountEntity) {
        Set<FidoCredential> credentials = accountEntity.getCredentials().stream()
                .map(
                        c -> new FidoCredential(
                                c.getId(), c.getType(), accountEntity.getId(), c.getPublicKeyCose()))
                .collect(Collectors.toSet());

        return new UserAccount(
                accountEntity.getId(), accountEntity.getName(), accountEntity.getEmail(), credentials);
    }
    ...
    public void addCredential(FidoCredential fidoCredential) {
        ...
        UserAccountEntity account = this.userAccountRepository
